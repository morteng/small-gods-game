 ...
 */
export const WORLD_CONTENT_VERSION = 45;  // RIVER DENSITY TUNE: fewer rivers. Base flow threshold 500→560 + headwater taper fraction 0.22→0.40 (shorter wispy headwaters) + `buildWaterNetwork` now uses the same area-scaled+density threshold as the raster (was a fixed 500), so carve widths shrink consistently. New `WorldStyle.riverDensity` knob (default 1 = neutral) scales the threshold INVERSELY through `generateHydrology`/`buildWaterNetwork`. River raster + carved channels change ⇒ discard older autosaves. See prior notes below.
