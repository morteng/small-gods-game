 ...
 */
export const WORLD_CONTENT_VERSION = 31;  // MANOR PREMISES: the manor buildingType gains 'stabling'/'water-supply' requirements, so the site-expansion pass now derives a stable + a yard well for every placed manor — generalising the premises machinery (previously tavern-only) to a second, prominent focus building. Purely additive (existing layout byte-identical; the stable/well are appended) but worldgen output gains entities ⇒ discard older autosaves. (30 = site FIXTURES — wells co-placed; 29 = site expansion E2 auxiliaries; 28 = site-fitness live-wiring; 27 = barrier gates over visual extent C1; 26 = diagonal bridges; 25 = riverside levee #24; 24 = parallel-road merge; 23 = entrance stoops; 22 = river density area-scaling; 21 = aqueduct arcade G6 polish; 20 = emergent aqueducts G6 on piers; 19 = stairs CONNECT G3d.)
