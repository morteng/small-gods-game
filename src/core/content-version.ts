 ...
 */
export const WORLD_CONTENT_VERSION = 47;  // BIOME BY ABSOLUTE HEIGHT+SLOPE: mountain/peak/ice now gate on real metres above sea (elevation·reliefM) + local slope, not a fraction of the local relief — so a low-relief world stops classifying (and snow-capping) a 6 m bump as alpine mountain/rock. Default-relief (48 m) worlds keep their old mountain/peak extent (thresholds calibrated to the old fractions). Biome→tile output shifts slightly ⇒ discard older autosaves. See prior notes below.
