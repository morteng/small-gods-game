 ...
 */
export const WORLD_CONTENT_VERSION = 50;  // IRRIGATION (G7): each farm_field patch within reach of water gets an `irrigation_ditch` conveyance dug across the open soil to the nearest stream/lake, and its served fields are flagged `irrigated`. New tile type + tile output changes ⇒ discard older autosaves. See prior notes below.
