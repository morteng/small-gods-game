 ...
 */
export const WORLD_CONTENT_VERSION = 60;  // SEA ARCHES PLACED (first natural-landform MESH prop in the world): worldgen now scatters a few `sea_arch` entities on steep rocky coasts (`buildCoastalLandmarks`, emergent from the biome map, sparse + deterministic, capped 4). A sea arch is a rock ring with a real hole bored through it — a landform the single-valued heightfield physically cannot represent — so it's a generative self-lit mesh prop (`sea_arch` plant preset → composeStructure, no art), foot-z lifted onto the terrain. New worldgen entities ⇒ discard older autosaves. See prior notes below.
