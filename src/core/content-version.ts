 ...
 */
export const WORLD_CONTENT_VERSION = 30;  // SITE FIXTURES (E2-fixtures): the site-expansion pass now realises ground/façade FIXTURES too, not just auxiliary buildings — a fixtureType that names a prop blueprint (the catalogue 'well' ↔ the 'well' civic prop) is co-placed in the yard, so a tavern now gets a yard well (from its 'water-supply' requirement) beside its stable. Data-only tokens (signage/seating) stay graph-only. Strictly additive (aux placement byte-identical; wells appended) but worldgen output gains prop entities ⇒ discard older autosaves. (29 = site expansion E2 auxiliaries; 28 = site-fitness live-wiring; 27 = barrier gates over visual extent C1; 26 = diagonal bridges; 25 = riverside levee #24; 24 = parallel-road merge; 23 = entrance stoops; 22 = river density area-scaling; 21 = aqueduct arcade G6 polish; 20 = emergent aqueducts G6 on piers; 19 = stairs CONNECT G3d.)
