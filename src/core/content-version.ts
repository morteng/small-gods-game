 ...
 */
export const WORLD_CONTENT_VERSION = 46;  // RIVER WIDTH TAPER: the painted river raster + the render water mask now stamp at the per-vertex channel width (W ∝ √Q, `reachHalfWidths`) the carve already used — thin spring → broad mouth, stepping up at confluences — instead of a flat per-class halfWidth. River tile footprint changes (walkability/placement) ⇒ discard older autosaves. See prior notes below.
