 ...
 */
export const WORLD_CONTENT_VERSION = 48;  // FARM FIELDS: farm buildings (barn/stable) now till a patch of `farm_field` tiles on the open soil beyond them — the live noise worldgen had none (farm_field was WFC-only). Walkable ground, so placement/roads unaffected, but tile output changes ⇒ discard older autosaves. See prior notes below.
