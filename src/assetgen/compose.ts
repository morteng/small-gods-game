...
type PartVariant =
  ...
  | { prim: 'rock'; center: [number, number]; baseZ: number; radius: number; seed: number; jitter?: number; aspect?: number; mat?: Mat; subdiv?: number; cuts?: number }
...
export async function partFacets(p: Part): Promise<{ facets: WorldFacet[]; anchors?: BuildingAnchors; linearAnchors?: LinearWorldAnchors }> {
  switch (p.prim) {
    ...
    case 'waterwheel': return { facets: manifoldToFacets((await solidWaterwheel(p.center, p.radius, p.width, p.axis, p.spokes, p.paddles)).getMesh(), p.material ?? 'timber', p.work, undefined, p.finish, p.tint) };
    case 'ellipsoid': {
    ...
    case 'rock':      return { facets: rockFacets({ center: p.center, baseZ: p.baseZ, radius: p.radius, seed: p.seed, jitter: p.jitter, aspect: p.aspect, mat: p.mat, subdiv: p.subdiv, cuts: p.cuts }) };
