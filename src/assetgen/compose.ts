...
export type Part =
  | { prim: 'box'; at: Vec3; size: Vec3; material?: Mat; work?: string; apertures?: ApertureBox[]; yaw?: number }
  | { prim: 'cylinder'; center: [number, number]; baseZ: number; radius: number; height: number; material?: Mat; work?: string; apertures?: ApertureBox[] }
  ...
  | { prim: 'building'; wings: Wing[]; wallMat?: Mat; roofMat?: Mat; roofStyle?: RoofStyle; features?: BuildingFeatures; seed?: number; apertures?: ApertureBox[]; wallWork?: string; baseCourse?: number; cutaway?: boolean }
...
async function partFacets(p: Part): Promise<{ facets: WorldFacet[]; anchors?: BuildingAnchors; linearAnchors?: LinearWorldAnchors }> {
  switch (p.prim) {
    ...
    case 'building':  return buildingFacets(p.wings, p.wallMat, p.roofMat, p.roofStyle, p.features, p.seed, p.apertures, p.wallWork, p.baseCourse, p.cutaway);
