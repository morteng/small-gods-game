...
export interface SolverOptions {
  ...
  onBacktrack?: ((backtracks: number) => void) | null;
}
...
export class Solver {
  ...
  options: {
    ...
    onBacktrack: SolverOptions['onBacktrack'];
  };
  rng: () => number;
  backtracks: number;
  iterations: number;

  constructor(grid: Grid, propagator: Propagator, options: SolverOptions = {}) {
    this.grid = grid;
   ...
   */
  async solveAnimated(stepsPerFrame: number = 50): Promise<SolveResult> {
