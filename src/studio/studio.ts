...
export function mountObjectStudio(container: HTMLElement, opts: ObjectStudioOpts = {}): StudioHandle {
  ...
  function setPickHover(key: string | null, e?: MouseEvent): void {
    ...
    const r = viewPane.getBoundingClientRect();
    // A vent's chip hints the hearth-toggle click affordance instead of just naming the part —
    // `ventScreenPoints` is the one place `id` gets attached, so this lookup can't disagree with
    // what the click handler below treats as "a vent".
    const isVent = ventScreenPoints().some((v) => v.id === key);
    pickChip.textContent = isVent ? `${key} · click to ${isVentLit(key) ? 'snuff' : 'light'}` : key;
  ...
  canvas.addEventListener('mouseup', (e) => {
    ...
    if (!key) return;
    // A click on a chimney/vent toggles ITS hearth independent of the global temperature dial
    // (see the hearthOverrides block above); any other feature just selects in the tree, as before.
    if (ventScreenPoints().some((v) => v.id === key)) hearthOverrides.set(key, !isVentLit(key));
    treeSelect(key);
  });
