...
export function plantSpriteItemFromPack(
  ...
  if (buryPx > 0) {
    ...
    // Amplitude floors at 4 px and tracks ~70 % of the buried depth: the old 2 px floor
    // left shallow-buried rocks with a visually STRAIGHT cut — a two-pixel wave reads as
    // a razor line at gameplay zoom (user report, dusty scree slopes where no thick snow
    // contact band hides the edge). Capped at 0.16 of the drawn height: deeper waves on a
    // big boulder read as sawtooth fangs hanging under the rock, not soil banking over it.
    item.scallop = Math.min(0.16, Math.max(4, buryPx * 0.7) / Math.max(1, visH));
  }
  if (pack.shadow) {
