...
export function traceRiverPolylines(map: GameMap, hydro: HydrologyResult): RiverPath[] {
  ...
  for (let i = 0; i < N; i++) {
    ...
    for (let step = 0; step < N; step++) {
      ...
      const t = drainTo[c];
      if (t < 0) break;                            // outlet
      if (waterType[t] !== WaterType.River) { pts.push(center(t)); break; } // mouth → lake/ocean
      c = t;
    }
    if (pts.length >= 2) paths.push({ points: pts, order });
  }
  return paths;
}
...
export function buildRiverRibbonMesh(map: GameMap, hydro?: HydrologyResult): RibbonMesh {
  ...
  const specs: RibbonSpec[] = paths.map((p) => ({
    ...
    halfWidth,
    speed,
    tag: [p.order, 1], // tag.y = 1 → river fragment program
    simplifyTol: 1.2,
  }));
