...
describe('door feature — scale-contract sizing', () => {
  it('default door derives height from DOOR_HEIGHT_TILES', () => {
    const { params } = doorFeatureType.resolve({ type: 'door', face: 'south' }, ctx);
    expect(params.height).toBeCloseTo(DOOR_HEIGHT_TILES, 5);
  });
  ...
  it('main door is a touch wider/taller but stays human-relative (< 1.4× human headroom)', () => {
    ...
    expect(params.height as number).toBeGreaterThan(DOOR_HEIGHT_TILES);
    expect(params.height as number).toBeLessThan(DOOR_HEIGHT_TILES * 1.4);
  });
