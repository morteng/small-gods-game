...
**Status:** The core arc is shipped to `main` — Spec A (spine), Time (Spec B/C), Phase 7 (NPC sim), Phase 8 (divine actions), Track 1 belief loops, D1 (mortality/birth/lineage), D2 (deterministic time-skip), Phase 9 (LLM backfill: client/prompt/writeback + provider config + live-apply). Since then: parametric building pipeline (blueprint→manifold→img2img→SpritePack), metric scale, settlement growth (S1–S6), a **WebGPU-only renderer**, GPU terrain + world connectome, and a **WebGPU-native immediate-mode UI** (`src/render/ui/`, the default chrome). `game.ts` is a thin coordinator (~1170 LOC) over 19 `src/game/` modules. **~3400 tests (~530 files).** Remaining gameplay: rivals (Track 3), Fate/DM (Track 4, `llmClientCapable` seam built but uncalled), conversation UI.
...
Phases 7–8 shipped; Phase 9 (LLM backfill) largely shipped; rivals (Track 3) and Fate (Track 4) ahead — see [ROADMAP.md](docs/ROADMAP.md).
...
- **Rival spirits** ⬜ (Track 3) — wire the inert `rival-spirit.ts` into a live `RivalSystem`; claim the prayers you don't answer.
- **Fate / DM agent** ⬜ (Track 4) — background LLM on the **capable tier** (`DEFAULT_CAPABLE_MODEL`) via `Game.llmClientCapable`: pacing, plot threads, escalation, anti-snowball; also the era-authoring half of the D2 skip loop. The codebase calls this layer **"Fate."**
</content>
