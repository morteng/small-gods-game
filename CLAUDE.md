...
**Status:** The core arc is shipped to `main` — Spec A (spine), Time (Spec B/C), Phase 7 (NPC sim), Phase 8 (divine actions), Track 1 belief loops, D1 (mortality/birth/lineage), D2 (deterministic time-skip), Phase 9 (LLM backfill: client/prompt/writeback + provider config + live-apply). Since then: parametric building pipeline (blueprint→manifold→img2img→SpritePack), metric scale, settlement growth (S1–S6), a **WebGPU-only renderer**, GPU terrain + world connectome, and a **WebGPU-native immediate-mode UI** (`src/render/ui/`, the default chrome). `game.ts` is a thin coordinator (~1170 LOC) over 19 `src/game/` modules. **~3630 tests (~554 files).** Rivals (Track 3) core shipped — live `RivalSystem` on real data + unanswered-prayer claiming. Remaining gameplay: Fate/DM (Track 4, `llmClientCapable` seam built but uncalled), conversation UI.
