...
**GOTCHA — the frame loop now runs DURING worldgen** (it starts with the shell so the title backdrop animates). `bootstrapWorld` sets `state.map`/`state.world` *partway through*, before it seeds cohorts, installs the weather stepper and builds the flood watch — so **"map exists" is NOT "world is ready"**. The sim advance gates on `Game.worldReady` (set in `onWorldReady`, which is where the loop used to start), never on `state.map`. Anything else that must not run against a half-built world gates the same way. Pinned by `tests/unit/bootstrap-explicit-fresh.test.ts`.

**A world is ENTERED RUNNING** — `Game.enterWorldRunning()` states the sim rate at world-ready (clears a hard pause, resets the stashed-rate fields, sets rate 1) and LOGS if it had to correct a non-default rate. Deliberate: now that a session can go title → demo → quit-to-title with no reload, the pause menu / modal cards / hard pause all legitimately zero the rate, and a world that opens frozen reads as broken. `returnToTitle()` leaves the rate at 0 — no world to advance while the title is up.
