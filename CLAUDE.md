...
**Current Status:** The core arc is shipped to `main` — Spec A (spine), Time (Spec B/C), Phase 7 (NPC sim), Phase 8 (divine actions), the Dilemma MVP / Track 1 belief loops, D1 (mortality/birth/lineage), D2 (deterministic time-skip), and Phase 9 (LLM backfill: client/prompt/writeback + player provider config + live-apply). Built on top since: the parametric building pipeline (blueprint→manifold→img2img→SpritePack, geometry **v13**), metric scale standardization, settlement growth (S1–S6), water hydrology (S0–S6), flora generation (L-system + 26-species fact-DB + proctree/space-colonization generators), belief-granted powers + the divine inbox, the storylet engine, a **WebGPU-only renderer** (the Canvas2D/PixiJS scene path was retired) with GPU terrain + a world connectome + a **WebGPU-native immediate-mode UI** (`src/render/ui/`, now the default chrome), and a **unified Object+World studio shell** (`?studio=…`: world browser, drill-down, turntable, live param editing). `game.ts` is a thin coordinator (~386 LOC) over `src/game/` modules. **~2640 tests passing (~436 files).** The single forward plan is `docs/ROADMAP.md`; remaining gameplay-arc work = conversation UI (Track 2), rivals (Track 3), Fate (Track 4 — capable-tier invocation via the `llmClientCapable` seam, built but uncalled), progression/win-state (Track 5), the Book (Track 6). For live session state / active engine epics, see the session memory (`MEMORY.md`), not this file.
