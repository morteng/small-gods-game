...
- **Building sprites are generated TEXT-ONLY (no init_image).** `scripts/gen-buildings.ts` runs descriptor → `buildingBrief` → `PixfluxCompiler` → PixelLab pixflux using the model's real controls (`isometric` + `view:'high top-down'` + `text_guidance_scale` + `negative_description` + a per-building `color_image` palette). **No init_image:** pixflux is img2img, so ANY init (a 3D massing block OR a placement scaffold) gets reproduced flat — text-only lets the model draw a properly detailed isometric building. The old offline 3D/scaffold guide renderers (three.js + headless Chrome, and a 2D scaffold) were tried and removed; `no-three-in-bundle.test.ts` keeps three/gl out of the bundle. Roof vocabulary + height model still live in `building-massing-model.ts` (`ROOF_PROFILES`/`roofRise`) for the IN-GAME parametric massing fallback. Paid regen is user-run: `gen-buildings.ts` (needs `PIXELLAB_API_KEY`) → `seed-base-library.mjs`.
