...
**P3 — Surfaces.** ✅ **SHIPPED 2026-07-01 (P3.7 hover popover + P3.8 inspector).**
...
8. ✅ **WebGPU inspector SHIPPED.** Right-docked panel for the current selection (npc, else a selected building's settlement — the causal site keeps its own card). Data = `GameQuery.inspect(target)` (`game-query.ts`): full legible state as 0–1 bars (belief scalars + mood + needs), a **"THEY BELIEVE YOU COMMAND"** section (per-domain `getDomainBelief` for an NPC; aggregate `aggregateDomain` conviction for a settlement — the belief-loop feedback), and the **full divine vocabulary** via `affordancesForTarget` (locked/unaffordable verbs greyed, castable ones fire on the selection). Verb-labelled buttons (target-first → no verbose `describe()`). Content budgets (no scroll yet): the ACTS block is height-reserved so it always fits; state/domains flow into the remainder. Camera cluster tucks left (`drawCameraCluster(rightInset)`) so the two never overlap. Game glue mirrors the hover path: `inspectorTarget`/`inspectorView`/`castInspector` + a frozen target; hooks `getInspector`/`onInspectorCast`/`onCloseInspector`. +11 tests (5 query, 6 runtime); rendered + verified live (Tola: 8 bars, 2 domains, WHISPER/ANSWER PRAYER/PROBE MIND/DREAM + SMITE🔒).

**P4 — The card (keystone). ✅ SHIPPED 2026-07-01.**
9. ✅ `UiSpec` + `validateUiSpec` (`src/story/uispec.ts`): a CLOSED/enumerated card — `title` + a walk of `UiSpecBlock` (paragraph/npcLine/omen/divider/beliefBar) + `UiSpecChoice[]` (each a pre-paired `Command`). `validateUiSpec` CLAMPS to no-scroll budgets (title/blocks/chars/choices; belief-bar 0–1) rather than rejecting — total, never throws. `renderUiSpec` walker in `ui-runtime.ts` presents it as a centred modal card (`presentUiSpec(spec,onChoose)`; `hasCard()`); content BUDGETS (choice stack bottom-reserved, body flows into the remainder); Esc / backdrop-click cancels, a choice hands its command back + dismisses. Modal like the story card (pauses via `onStoryToggle`, clears any hover), mutually exclusive with a running `StorySession`.
10. ✅ Whisper card (`src/game/affordance/whisper-card.ts`): `buildWhisperCard(target,source,ctx)` — sim-owned, synchronous, deterministic (`Math.random`-free), keyed by `dominantNeed × activeEvent × dominantDomainBelief`. Always 2–3 paths: soothe-the-need (always), name-the-omen (if an ominous event grips the home), affirm-the-domain (if they already lean toward a power) — a generic affirm backfills to guarantee ≥2. Each path pre-pairs a real `whisper` `Command` carrying the words in `payload.text` + the steer in `params.slant`. The RENDERED spec holds resolved fallback strings (replay-safe, zero LLM in replay); LLM prose-enrichment is a later, non-replay pre-warm seam. Card is presented live and re-derives deterministically from world state (choice `Command` is queued with `seq`).
11. ✅ **Smite** thunderbolt FX: added a `smite` effect (`divine-effects.ts` — jagged strike-down bolt + white impact flash + scorch ring, deterministic zigzag). Fired via the new unified `Game.emitDivine(verb,target)` seam that ALL player surfaces (hover/inspector/reticle/inbox/powers) route through — branch verbs (`whisper`) open the card; leaf verbs emit + `fireCastFx` (smite → `divineEffects.trigger('smite',…)` at the resolved world tile). +19 tests (5 uispec, 7 whisper-card, 4 runtime card + wiring); tsc + build green.
