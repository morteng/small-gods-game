...
| 11 | ✅ **Closed.** Faith responds to need **direction**, not just magnitude: `tickNpcEntity` (`src/sim/npc-sim.ts`) keys both terms on the **worst** need (`lowestNeed()`) — desperation fires when *any* axis collapses, comfort decay only when *every* need is met, so one collapsing axis suspends secularization. `lowestNeed` breaks ties in the same order as `prayerSubject`'s `NEED_KEYS`, so the need a mortal is desperate about is the need they pray about. `computeMood()` survives as the mood/UI scalar. The no-op case above now moves faith (`tests/unit/need-direction.test.ts`). Shipped as S2b.2 of `docs/superpowers/plans/2026-08-01-interaction-scaling-plan.md`. | — |
| 12 | ✅ **Closed.** `worship` fires on the **lowest** need past its per-need threshold (`WORSHIP_THRESHOLDS`, `prayerSubject()` in `src/sim/systems/npc-activity-system.ts`), not just `meaning`. Every prayer carries a **subject** (`props.prayerNeed`): `answer_prayer` (`src/sim/divine-actions.ts`) restores the need actually asked for, and rival claims (`src/sim/rival-claims.ts`) domain-match against it. | — |
